...
public class LogUtil {
  ...
  public static String buildMessage(final String message) {
    return initMessageBuilder().message(message).build();
  }

  public static String buildMessagePayload(final String payload) {
    return initMessageBuilderWithUriAndMethod().payload((payload)).build();
  }
  ...
  private static MessageBuilder initMessageBuilder() {
    return new MessageBuilder().uuidRequest(RequestUtil.getUuidRequest()).and().appName("CoopVote");
  }

  private static MessageBuilder initMessageBuilderWithUriAndMethod() {
    return initMessageBuilder().uri(RequestUtil.getUri()).and().method(RequestUtil.getMethod());
  }
